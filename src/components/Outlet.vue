...
<script setup>
import { onMounted, onUnmounted, defineProps, defineEmit, watch } from 'vue'
...
.layer:nth-child(1) {
  transform: perspective(1000px) rotateX(1deg) translateZ(-1px) scaleX(0.8);
}

.layer:nth-child(2) {
  transform: perspective(1000px) rotateX(-1deg) translateZ(-5px) scaleX(0.8);
}
...
.layer i {
  ...
  width: 30px;
  height: 150px;
  background: linear-gradient(#3dfff8, #3dfff800);
